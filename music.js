  ...
  async execute(client) {
    client.on('message', async (msg) => {
      ...
      if (command === 'play' || command === 'p') {
        ...
        if (searchKeysUrl == '')
          return msg.reply('You need to provide a search key or youtube url');
        ...
        if (songQueue.length == 0)
          return msg.channel.send('The song queue is currently empty');
        ...
        totalTime =
          str_pad_left(totalMinutes, '0', 2) +
          ':' +
          str_pad_left(totalSeconds, '0', 2);
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setFooter(
            `${songQueue.length} songs in queue | Total length: ${totalTime}`,
            msg.guild.iconURL()
          );
        ...
        if (songQueue.length === 1)
          return msg.reply('You are skipping the last song!');
        ...
        if (index == 0)
          return msg.reply(
            'To remove current song why not use "skip" instead :joy:'
          );
        if (index > songQueue.length - 1)
          return msg.reply('The number of the song not found!');
    ...
    async function playSong(msg, connection, voiceChannel) {
      ...
      msg.channel.messages
        .fetch({ around: `${nowPlayingEmbed}`, limit: 1 })
        .then((messages) => {
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .setFooter(
          `Requested by ${currentTrack.username}`,
          `${currentTrack.userpic}`
        );
    ...
    async function addSongtoQueue(url, msg, connection, voiceChannel) {
      ...
      const songDuration =
        str_pad_left(songMinutes, '0', 2) +
        ':' +
        str_pad_left(songSeconds, '0', 2);
      const song = {
